...
Example script to demonstrate the usage of the flux_simulator_module for a single
atmosphere clear sky simulations.
...
# Here we use a LUT based the WSM abs_lookupSetup (see ARTS 2.6 doumentation).
# If you want to be sure that the LUT is recalculated, set recalc=True. 
LW_flux_simulator.get_lookuptable(atm, recalc=False) 
...
# Here we use a LUT based the WSM abs_lookupSetup (see ARTS 2.6 doumentation).
# If you want to be sure that the LUT is recalculated, set recalc=True. 
SW_flux_simulator.get_lookuptable(atm, recalc=False) 
